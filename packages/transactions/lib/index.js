...
function _serializePow(transaction, signature) {
    ...
    return (0, bytes_1.hexConcat)(["0x04", RLP.encode(fields)]);
}
...
function _serializeDeposit(transaction, signature) {
    ...
    // Type identification is inserted at the begin
    return (0, bytes_1.hexConcat)(["0x06", RLP.encode(fields)]);
}
function _serializeNested(transaction, signature) {
    ...
    return (0, bytes_1.hexConcat)(["0x07", RLP.encode(fields)]);
}
...
function _parsePow(payload) {
    ...
    if (transaction.length !== 12) {
        logger.throwArgumentError("invalid component count for transaction type: 4", "payload", (0, bytes_1.hexlify)(payload));
    }
    var tx = {
        type: 4,
    ...
    if (v && v !== "0x" && r && r !== "0x" && s && s !== "0x") {
        try {
            tx.v = bignumber_1.BigNumber.from(v).toNumber();
        }
        catch (error) {
            logger.throwArgumentError("invalid v for transaction type: 4", "v", v);
        }
        tx.r = (0, bytes_1.hexZeroPad)(r, 32);
        tx.s = (0, bytes_1.hexZeroPad)(s, 32);
        tx.hash = (0, keccak256_1.keccak256)(payload);
...
function _parseDynamicCrypto(payload) {
    ...
    if (transaction.length !== 13 && transaction.length !== 16) {
        logger.throwArgumentError("invalid component count for transaction type: 5", "payload", (0, bytes_1.hexlify)(payload));
    }
    var tx = {
        type: 5,
        chainId: handleNumber(transaction[0]).toNumber(),
...
function _parseDeposit(payload) {
    var transaction = RLP.decode(payload.slice(1));
    if (transaction.length !== 13 && transaction.length !== 16) {
        logger.throwArgumentError("invalid component count for transaction type: 6", "payload", (0, bytes_1.hexlify)(payload));
    }
    var tx = {
        type: 6,
        chainId: handleNumber(transaction[0]).toNumber(),
