...
function _serializePow(transaction: UnsignedTransaction, signature?: SignatureLike): string {
    ...
    const fields: any = [
        formatNumber(transaction.nonce || 0, "nonce"),
        formatNumber(transaction.maxPriorityFeePerGas || 0, "maxPriorityFeePerGas"), // GasTipCap
        formatNumber(transaction.gasLimit || 0, "gasLimit"),
        ((transaction.to != null) ? getAddress(transaction.to) : "0x"),
        formatNumber(transaction.value || 0, "value"),
        (transaction.data || "0x")
...
function _serializeDynamicCrypto(transaction: UnsignedTransaction, signature?: SignatureLike): string {
    if (transaction.gasPrice != null) {
        const gasPrice = BigNumber.from(transaction.gasPrice);
        const maxFeePerGas = BigNumber.from(transaction.maxFeePerGas || 0);
        if (!gasPrice.eq(maxFeePerGas)) {
...
function _parsePow(payload: Uint8Array): Transaction {
    ...
    const tx: Transaction = {
        ...
        maxPriorityFeePerGas: handleNumber(transaction[1]), // GasTipCap
        gasPrice: handleNumber(transaction[1]), // Use GasTipCap as gasPrice
